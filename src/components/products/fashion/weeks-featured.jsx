    ...
    content = (
      <Swiper {...slider_setting} modules={[Navigation]} className="tp-featured-slider-active swiper-container">
        {product_items.map((item) => {
          const {id, img, title, price, discount, reviews } = item || {};
          return (
            <SwiperSlide key={item.id} className="tp-featured-item white-bg p-relative z-index-1">
              ...
              <div className="tp-featured-content">
                ...
                <div className="tp-featured-btn">
                  <Link href={`/product-details/${id}`} className="tp-btn tp-btn-border tp-btn-border-sm">Shop Now
                    {" "}<ArrowRightLong />
                  </Link>
                </div>
              </div>
