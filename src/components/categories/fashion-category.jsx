...
const FashionCategory = () => {
  ...
  // go to full category page
  const goToFullCategory = () => {
  ...
  if (!isLoading && !isError && categories?.result?.length > 0) {
    const category_items = categories.result.slice(0, 6); // ✅ chỉ lấy 6
    ...
    content = (
      <>
        ...
        <div className="col-12 text-center mt-30">
          <button
            onClick={goToFullCategory}
            className="tp-btn tp-btn-border tp-btn-border-sm"
          >
            See More Categories <ArrowRightLong />
          </button>
        </div>
      </>
    );
